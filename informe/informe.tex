...
ataques dirigidos.} El umbral de percolación cae de $f_c \approx 0{,}237$ (fallo
aleatorio) a $f_c \approx 0{,}023$ (ataque por grado o por intermediación recalculada):
eliminar deliberadamente 4 de 177 equipos hace lo que al azar exige eliminar 42. La
eficiencia global se degrada aún antes: con solo el 5\,\% de los nodos atacados cae al
14,8\,\% de su valor original.

\item \textbf{La fragilidad de esta red es de conectividad, no de sobrecarga en cadena.}
Bajo el modelo de cascada de Motter-Lai ningún nodo individual desencadena un colapso
mayor al 8,5\,\% de la red, ni siquiera con margen de tolerancia $\tau = 0$. Son dos
mecanismos de falla distintos y esta red solo exhibe uno.
...
La matriz de confusión comunidad $\times$ campus (Anexo, \code{p4\_matriz\_confusion\_comunidad\_campus.csv})
es casi bloque-diagonal: \textbf{Louvain no mezcla campus entre sí}, pero \textbf{sí
subdivide cada campus grande en varias comunidades}. Campus Central se reparte en once
comunidades, Balzay en cuatro y Paraíso en cinco. De ahí la aparente contradicción de los
dos índices: el NMI es moderado (0,584) porque la partición de Louvain \textit{refina} la de
campus y comparte con ella mucha información; el ARI es bajo (0,190) porque penaliza
específicamente la sobre-partición, y aquí la hay por construcción. Los dos índices no se
contradicen: miden cosas distintas y ambos son correctos.
...
\toprule
\textbf{Campus} & \textbf{Nodos de acceso} & \textbf{Flujo máx.\ (Mbps)} & \textbf{Aristas del corte} & \textbf{Cap.\ del corte} & \textbf{Iter.\ FF / EK} \\
\midrule
Campus Central        & 56 & 44\,000 &  7 & 44\,000 & 44 / 44 \\
Campus Balzay         & 24 & 23\,000 & 23 & 23\,000 & 23 / 23 \\
Sede Museo            &  1 & 11\,000 &  2 & 11\,000 &  3 / 3 \\
Sede Centro Histórico &  1 & 11\,000 &  2 & 11\,000 &  4 / 2 \\
Campus Yanuncay       & 11 & 10\,000 &  1 & 10\,000 & 10 / 10 \\
Campus Hospitalidad   &  4 &  4\,000 &  4 &  4\,000 &  4 / 4 \\
\textbf{Campus Paraíso} & \textbf{35} & \textbf{1\,000} & \textbf{1} & \textbf{1\,000} & \textbf{1 / 1} \\
\bottomrule
\end{tabular}
\end{adjustbox}
\end{table}
...
\midrule
$f_c$ (donde $S(f)$ cruza 0,5) & 0,237 & 0,023 & 0,045 & 0,023 \\
Nodos eliminados en $f_c$      & 42    & 4     & 8     & 4 \\
$S(f = 0{,}05)$                & 0,879 & 0,381 & 0,212 & 0,053 \\
\bottomrule
\end{tabular}
\end{adjustbox}
\end{table}
...
23,7\,\% de sus nodos al azar ---42 equipos--- antes de que la componente gigante caiga a la
...
38,1\,\% de la red conectada, mientras que el de intermediación recalculada deja solo el
5,3\,\%. Recalcular la intermediación después de cada eliminación es, con diferencia, la
...
El ataque dirigido a enlaces baja $f_c$ de \textbf{0,354} (aleatorio) a \textbf{0,321}
(puentes primero): una caída del 9,3\,\%, mucho menor que la observada en nodos. La razón es
...
\midrule
Fallo aleatorio            & 0,959 & 0,785 & 0,598 & 0,388 \\
Ataque por intermediación  & 0,611 & 0,148 & 0,057 & 0,012 \\
\bottomrule
\end{tabular}
\end{adjustbox}
\end{table}
...
61,1\,\% mientras $S(f)$ apenas se movió; con el 5\,\% cayó al 14,8\,\%.
...
\midrule
$f_c$ (fallo aleatorio) & 0,237 & 0,271 & 0,232 \\
\bottomrule
\end{tabular}
\end{adjustbox}
\end{table}
...
Prácticamente igual. El umbral real (0,237) es casi idéntico al del modelo de configuración
(0,232), que preserva exactamente la secuencia de grados, y algo inferior al de
Erdős--Rényi (0,271), que solo preserva $n$ y $m$. La conclusión es consistente con la de
P2: \textbf{la robustez de UCuenca ante fallos aleatorios se explica por su secuencia de
grados heterogénea ---unos pocos concentradores, muchas hojas--- y no por algún patrón
estructural adicional.} La red no es ni mejor ni peor de lo que su distribución de grado
predice; es exactamente lo que esa distribución implica. Curiosamente, el grafo aleatorio de
Erdős--Rényi resulta \textit{más} robusto que la red real: al no tener concentradores, no
tiene nada que perder en un fallo aleatorio con consecuencias desproporcionadas.
...
significativa hacia nadie) sobre una malla $\tau \in \{0;\, 0{,}05;\, 0{,}1;\, 0{,}15;\,
0{,}2;\, 0{,}3;\, 0{,}4;\, 0{,}5;\, 0{,}75;\, 1{,}0\}$.

\begin{table}[H]
\centering
\caption{Fracción máxima de la red afectada por una cascada, por margen de tolerancia $\tau$}
\label{tab:cascada}
\small
\begin{adjustbox}{max width=\textwidth}
\begin{tabular}{lcccccccccc}
\toprule
$\tau$ & 0,00 & 0,05 & 0,10 & 0,15 & 0,20 & 0,30 & 0,40 & 0,50 & 0,75 & 1,00 \\
Máx.\ afectado & 7,3\,\% & 6,8\,\% & 6,2\,\% & 6,8\,\% & 8,5\,\% & 7,9\,\% & 7,9\,\% & 6,8\,\% & 6,8\,\% & 6,2\,\% \\
\bottomrule
\end{tabular}
\end{adjustbox}
\end{table}

\textbf{Determinación del margen crítico $\tau_c$.} El enunciado pide el valor de $\tau$ por
debajo del cual la falla de un único nodo provoca una cascada que afecta a más del 20\,\% de
la red. \textbf{Ningún nodo candidato alcanza ese umbral en ninguno de los diez valores de
$\tau$ probados, ni siquiera con margen $\tau = 0$}, de modo que no existe un $\tau_c$
positivo que reportar. El peor caso observado es \code{PE2-BALZAY}, que a $\tau = 0{,}2$
arrastra al 8,5\,\% de la red (15 nodos). Le siguen \code{FORTIGATE-1800F-BALZAY} (7,3\,\%)
y el grupo \code{DATCC-2A-C3} / \code{INTERNET-MPLS} / \code{PE2-CENTRAL} /
\code{FORTIGATE-1800F-CENTRAL} (6,8\,\%). Reportar «$\tau_c = 0$» sería engañoso: se leería,
al revés, como «la red es frágil incluso sin margen».

\textbf{¿Contradice esto la fragilidad hallada en P8? No: son dos mecanismos de falla
distintos.} En P8 la métrica es \textbf{conectividad} ---qué se desconecta de qué---; aquí es
\textbf{sobrecarga} ---qué se satura por redistribución de tráfico. Esta red tiene poca
...
tumbarlo en cadena. Dicho de forma directa: \textbf{esta red se rompe por desconexión, no por
efecto dominó}. La distinción tiene consecuencias operativas concretas ---no hace falta
sobredimensionar capacidad «por si acaso» en previsión de cascadas, pero sí hace falta
redundancia topológica--- y es exactamente el tipo de conclusión que solo se obtiene
ejecutando ambos análisis.
...
\begin{table}[H]
\centering
\caption{Ficha de los cinco puntos más críticos: función en la jerarquía y consecuencia estimada de su falla}
\label{tab:fichas}
\small
\begin{adjustbox}{max width=\textwidth}
\begin{tabular}{p{3.2cm}p{5.3cm}p{5.7cm}}
\toprule
...
fallos aleatorios ($f_c \approx 0{,}237$) pero extremadamente frágil ante ataques dirigidos
...
\item \textbf{La fragilidad de esta red es de conectividad, no de sobrecarga en cadena.}
Bajo el modelo de cascada de Motter-Lai, ningún nodo individual desencadena un colapso mayor
al 8,5\,\% de la red, ni siquiera con margen $\tau = 0$. Es una distinción operativa de
consecuencias directas: hace falta redundancia topológica, no sobredimensionamiento de
capacidad preventivo.
